                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" value="{{count($task->name) ? $task->name : ''}}" class="form-control" id="name" placeholder="введите название">
                                </div>
                            </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <textarea id="description"  name="description" rows="9" class="form-control my-editor" placeholder="Content..">{{count($task->description) ? $task->description : ''}}</textarea>
                                </div>
                            </div>
                                    ...
                                    <select id="type" name="status_id" class="form-control">
                                        @if(count($row['status']))
                                            @foreach($row['status'] as $item)
                                                <option {{$item->id == $task->status_id ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                    ...
                                    <select id="type" name="type_id" class="form-control">
                                        @if(count($row['type']))
                                            @foreach($row['type'] as $item)
                                                <option {{$item->id == $task->type_id ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                    ...
                                    <select id="type" name="user_id" class="form-control">
                                        @if(count($row['users']))
                                            @foreach($row['users'] as $item)
                                                <option {{$item->id == $task->user_id ? 'selected' : ''}} value="{{$item->id}}">{{$item->name}}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                            ...
                            <div class="col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input value="{{date('d.m.Y H:i',$task->ended_at_by_manager)}}" data-time-format=' hh:ii'  id="timepicker-actions-exmpl"  name="ended_at_by_manager" class="form-control" >
                                </div>
                            </div>
